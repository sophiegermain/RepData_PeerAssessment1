...
The mean of the total number of steps is `r mean` and the median is `r median`, reflecting the highest frequency of total steps in the [10,000; 15,000] bracket with a asymmetric distribution weighted to the left.
...
The 5-minute interval which contains the most steps on average across all days is interval `r top` with an average of `r max` steps.
...
Hence, the total number of rows with missing values is `r highest`.
...
The histogram has a more symmetric distribution compared to the original one. The mean of the total number of steps is now `r mean2` and the median is `r median2`. The equality of the mean and median again indicates symmetry of the distribution. This suggests that the initial asymmetry was caused by missing values, as many days had an understated number of total steps.
...
```{r panel, fig.width=7, fig.height=7, warning=FALSE}
library(lattice)
library(datasets)
library(plyr)
